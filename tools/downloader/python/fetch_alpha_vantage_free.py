#!/usr/bin/env python3
"""
Alpha Vantage Free Tier Data Fetcher for C++ Backtester

Downloads historical stock data from Alpha Vantage FREE API and saves it in CSV format
compatible with the C++ backtester.

Usage:
    python fetch_alpha_vantage_free.py SYMBOL API_KEY [OUTPUT_DIR]
    
Examples:
    python fetch_alpha_vantage_free.py AAPL YOUR_API_KEY
    python fetch_alpha_vantage_free.py MSFT YOUR_API_KEY data/
...
def fetch_alpha_vantage_free_data(symbol, api_key, output_dir="data"):
    """
    Fetch historical data from Alpha Vantage FREE API for a given symbol.
    ...
    """
    try:
        print(f"Fetching data for {symbol} from Alpha Vantage FREE API...")
        
        # Create output directory if it doesn't exist
        os.makedirs(output_dir, exist_ok=True)
        
        # Alpha Vantage FREE API endpoint (not premium)
        url = "https://www.alphavantage.co/query"
        
        # API parameters for FREE tier
        params = {
        ...
        print("Making API request to FREE endpoint...")
        ...
        if 'Note' in data:
            ...
            print("This might be a rate limit. Waiting 60 seconds...")
            time.sleep(60)
            return False
        ...
        for date, values in time_series.items():
            df_data.append({
                ...
                'Adj Close': float(values['4. close']),  # Use close as adj close for free tier
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Fetch historical stock data from Alpha Vantage FREE API",
...
Free tier: 5 calls per minute, 500 calls per day
    ...
    # Fetch data
    success = fetch_alpha_vantage_free_data(args.symbol, args.api_key, args.output_dir)
